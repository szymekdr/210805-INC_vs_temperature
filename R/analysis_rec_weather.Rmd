...
scatter3D(x, y, z, pch = 19, cex = 0.3, colvar = FALSE, col = "gray15", theta = 30, phi = 25, bty="u",ticktype="detailed",
          ...
          surf = list(x = x.pred, y = y.pred, z = z.pred,  
                      ...
                      fit = fitpoints,
                      border="black"), alpha = 0.10, zlim = c(0,8))
...
demography <- data %>%
  group_by(AREA, YEAR) %>%
  summarise(fl_mean = mean(FLEDGE, na.rm = T), rec_mean = mean(NEWRECRUIT, na.rm = T)) %>%
  filter((AREA == "OJ" | AREA == "FP" | AREA == "FA") & !is.nan(fl_mean) & !is.nan(rec_mean)) %>%
  # print(n = 50) %>%
  left_join(cf_nbox_occup, by = c("AREA" = "Plot", "YEAR" = "year")) %>%
  mutate(fl_mean_n = lag(fl_mean), rec_mean_n = lag(rec_mean))
...
summary(model14)
plot(model14)
hist(resid(model14, type = "deviance"))
