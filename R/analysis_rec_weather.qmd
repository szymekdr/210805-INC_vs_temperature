...
data_analysis <- data[,c("TEMP_BROOD", "PRCP_BROOD", "FRING", "YEAR", "CS", "sum_prcp_jan_march")]
data_analysis <- na.omit(data_analysis)
nrow(data_analysis)
data_analysis$YEAR2 <- as.factor(data_analysis$YEAR)
...
## Full Gotland data: extended population analyses with all nestboxes
...
model14 <- glm(no_pairs ~ -1 + scale(YEAR, scale = FALSE) * AREA, data = demography, family = "poisson")
summary(model14)
...
model15 <- glm(no_pairs ~ AREA * fl_mean_n, data = demography, family = "poisson")
summary(model15)

model16 <- glm(no_pairs ~ AREA * rec_mean_n, data = demography, family = "poisson")
summary(model16)
...
## Full Gotland data: Checking for temporal trend in clutch size
